...
/*
 * raw_expression_tree_mutator --- mutate raw parse trees
 *
 * This has exactly the same API as expression_tree_mutator, but instead of
 * mutating post-analysis parse trees, it knows how to walk the node types
 * found in raw grammar output.  (There is not currently any need for a
 * combined walker, so we keep them separate in the name of efficiency.)
 * Unlike expression_tree_walker, there is no special rule about query
 * boundaries: we descend to everything that's possibly interesting.
							...
							void *context)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_RangeVar:
		{
			...
			FLATCOPY(newnode, rangeVar, RangeVar);
			MUTATE(newnode->alias, rangeVar->alias, Alias *);
			return (Node *)newnode;
		}
		case T_SubLink:
		{
		...
		case T_NullTest:
		{
			...
			FLATCOPY(newnode, nullTest, NullTest);
			MUTATE(newnode->arg, nullTest->arg, Expr *);
			return (Node *)newnode;
		}
		case T_BooleanTest:
		{
			...
			FLATCOPY(newnode, booltest, BooleanTest);
			MUTATE(newnode->arg, booltest->arg, Expr *);
			return (Node *)newnode;
		}
		case T_JoinExpr:
		{
		...
		case T_NamedArgExpr:
		{
			NamedArgExpr *namedArgExpr = (NamedArgExpr *)node;
			NamedArgExpr *newnode;

			FLATCOPY(newnode, namedArgExpr, NamedArgExpr);
			MUTATE(newnode->arg, namedArgExpr->arg, Expr *);
			return (Node*)newnode;
		}
		case T_A_Indices:
		{
		...
		case T_A_ArrayExpr:
		{
			A_ArrayExpr *arryExpr = (A_ArrayExpr *)node;
			A_ArrayExpr *newnode;

			FLATCOPY(newnode, arryExpr, A_ArrayExpr);
			MUTATE(newnode->elements, arryExpr->elements, List *);
			return (Node*)newnode;
		}
		case T_ResTarget:
		{
		...
		case T_IndexElem:
		{
			...
			FLATCOPY(newnode, indelem, IndexElem);
			MUTATE(newnode->expr, indelem->expr, Node *);
			return (Node*)newnode;
